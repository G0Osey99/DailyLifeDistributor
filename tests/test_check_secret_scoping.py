...
def test_lint_flags_aliased_unscoped_call(tmp_path):
    """from core import secrets_store as _ss; _ss.get_secret(...) → flagged."""
    f = tmp_path / "bad.py"
    f.write_text(textwrap.dedent('''
        from core import secrets_store as _ss
        def boom():
            _ss.get_secret("youtube.token")
    '''))
    res = subprocess.run(
        [sys.executable, "scripts/check_secret_scoping.py", str(f)],
        capture_output=True, text=True,
...
def test_lint_passes_aliased_scoped_call(tmp_path):
    f = tmp_path / "good.py"
    f.write_text(textwrap.dedent('''
        from core import secrets_store as _ss
        def ok(org_id):
            _ss.get_secret("youtube.token", org_id=org_id)
    '''))
    res = subprocess.run(
        [sys.executable, "scripts/check_secret_scoping.py", str(f)],
        capture_output=True, text=True,
...
def test_lint_ignores_files_with_no_secrets_store_import(tmp_path):
    """A file that doesn't import secrets_store at all is skipped fast."""
